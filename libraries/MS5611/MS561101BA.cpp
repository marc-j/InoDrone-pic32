...
float MS561101BA::getPressure(uint8_t OSR) {
  ...
  int64_t off  = (((int64_t)C[1]) << 16) + ((C[3] * dT) >> 7);
  int64_t sens = (((int64_t)C[0]) << 15) + ((C[2] * dT) >> 8);
  if(rawPress != NULL) {
...
float MS561101BA::getTemperature(uint8_t OSR) {
  ...
  if(dT != NULL) {
    return ((1<<EXTRA_PRECISION)*2000l + ((dT * C[5]) >> (23-EXTRA_PRECISION))) / ((1<<EXTRA_PRECISION) * 100.0);
  }
  else {
    return NULL;
  }
...
int MS561101BA::readPROM() {
  for (int i=0;i<MS561101BA_PROM_REG_COUNT;i++) {
    ...
    if(Wire.available()) {
      C[i] = Wire.receive() << 8 | Wire.receive();
